...
export default class Event extends Base {
  ...
  mount (config) {
    ...
    Object.entries(config.vueInstance.item.events).forEach(([e, events]) => {
      events.map(event =>
        Object.entries(event).forEach(([f, eventHandler]) =>
          this.setterFactory
            ...
            .set({
              path: `on.${e}`,
              data: this.__eventHandler(event, f, eventHandler, config.vueInstance)
            }, config)
        )
      )
    })
  ...
  __eventHandler (config, getterLabel, getterConfig, vueInstance) {
    return (event) => {
  ...
  __getKeyCode (desc) {
    return Object.keys(keyCodes).find(key => keyCodes[key] === desc)
  }
}
